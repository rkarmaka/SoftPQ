# def _intersection_over_union(true_label, pred_label):
#     # Placeholder: Compute IoU matrix between labeled regions
#     pass
...
#             modified_mts_temp = iou_temp[iou_temp > iou_high][0] + np.sum(
#                 (iou_temp[((iou_temp > iou_low) & (iou_temp <= iou_high))]) / np.sqrt(overseg_count_temp + 1)
#             )
#         else:
#             modified_mts_temp = np.sum(
#                 (iou_temp[((iou_temp > iou_low) & (iou_temp <= iou_high))]) / np.sqrt(overseg_count_temp + 1)
#             )
#         modified_mts += modified_mts_temp
#     return modified_mtsdef _intersection_over_union(true_label, pred_label):
#     # Placeholder: Compute IoU matrix between labeled regions
#     pass
...
# 
########################################################
# from utils import _create_labeled_mask
    ...
    for i in range(1,n_gt+1):
        ...
        if iou_temp[iou_temp>iou_high].size>0:
            if overseg_count_temp>0:
                ...
                overseg_count_temp = 0
        if iou_temp[iou_temp>iou_high].size>0:
            modified_mts_temp = iou_temp[iou_temp>iou_high][0] + np.sum((iou_temp[((iou_temp>iou_low)&(iou_temp<=iou_high))])/np.log(overseg_count_temp+1))
        else:
            modified_mts_temp = np.sum((iou_temp[((iou_temp>iou_low)&(iou_temp<=iou_high))])/np.log(overseg_count_temp+1))
